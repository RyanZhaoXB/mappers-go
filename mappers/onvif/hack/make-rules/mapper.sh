...
function build() {
  ...
  GOOS=${os} GOARCH=${arch} CGO_ENABLED=1 \
    CGO_LDFLAGS="-L/usr/local/lib -ldflags -lavcodec -lavformat -lavutil -lswscale -lswresample -lavdevice -lavfilter" CGO_CFLAGS="-I/usr/local/include" \
    LD_LIBRARY_PATH=/usr/local/lib CGO_CFLAGS="-g -O2" \
    CGO_CPPFLAGS="" CGO_CXXFLAGS="-g -O2" CGO_LDFLAGS="-g -O2" \
    go build \
