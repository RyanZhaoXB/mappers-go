...
import (
	...
	"github.com/kubeedge/mappers-go/dmi-dev-framework/pkg/models"
...
// start the device.
func (d *DevPanel) start(ctx context.Context, dev *models.Device) {
...
func NewDevPanel(pd models.ProtocolDriver) *DevPanel {
	if devPanel == nil {
		devPanel = &DevPanel{
			deviceMuxs: make(map[string]context.CancelFunc),
			devices:    make(map[string]*models.Device),
			models:     make(map[string]models.DeviceModel),
			wg:         sync.WaitGroup{},
		}
	}

	if driver, ok := pd.(models.ProtocolDriver); !ok {
		klog.Fatal(fmt.Errorf("Please specify device interface"))
	} else {
